...
<tr>
<th align="left" style="height:5%;font-size:20px;">My Offers</th>
</tr>
<tr>
...
<?php
    ...
    <td bgcolor="#183EA5" align="right" colspan="2"><button class="btn" onclick="window.location.href='members.php?view=myOffers&add=true&N=<?php echo $info['No.']?>'"><font color="white">Add to WishList</font></button></td>
    </tr>
    <tr>
...
function addToWishList() {
	$check = mysql_query("SELECT * FROM product WHERE `No.`= '".mysql_escape_string($_GET['N'])."'")or die(mysql_error());
	while($info = mysql_fetch_array( $check )) {
		$username = $_COOKIE['ID_my_site'];
		...
		if ($check3 == 0) {
			...
			echo "window.location='members.php?view=myOffers'";
			echo "</script>";
			die();
		}
		else {
