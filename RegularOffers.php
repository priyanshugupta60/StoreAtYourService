...
<tr>
<th align="left" style="height:5%;font-size:20px;">Regular Offers</th>
</tr>
<tr>
...
<?php
    include 'connect.php';
    $check = mysql_query("SELECT * FROM productlist where discount>0 order by discount desc")or die(mysql_error());
    ...
    <td bgcolor="#183EA5" align="right" colspan="2"><button class="btn" onclick="window.location.href='members.php?view=myRegularOffers&add=true&N=<?php echo $info['No.']?>'"><font color="white">Add to WishList</font></button></td>
    </tr>
    <tr>
...
function addToWishList() {
	...
	while($info = mysql_fetch_array( $check )) {
		...
		if ($check3 == 0) {
			...
			echo "window.location='members.php?view=myRegularOffers'";
			echo "</script>";
			die();
		}
		else {
